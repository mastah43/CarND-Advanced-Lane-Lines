...
class LaneLine(object):
    ...
    def fit(self, nonzerox, nonzeroy, x_base:int, trace_img=None):
        ...
        fit_pix = np.polyfit(lane_pix_y, lane_pix_x, 2)
        fit_meters = np.polyfit(lane_pix_y * LaneImageSpec.ym_per_pix, lane_pix_x * LaneImageSpec.xm_per_pix, 2)

        # Trace: Draw fit poly
        if trace_img is not None:
            img_height = trace_img.shape[0]
            img_width = trace_img.shape[1]
            plot_y = np.linspace(0, img_height - 1, img_height, dtype=np.uint16)
            plot_x = LaneLine.x_for_fit(fit_pix, plot_y).astype(np.uint16)
            plot_yx = np.column_stack((plot_y, plot_x))
            plot_yx = plot_yx[np.logical_and(0 <= plot_yx[:,1], plot_yx[:,1] < img_width)]
            trace_img[plot_yx[:,0], plot_yx[:,1]] = [255, 255, 0]

        # Reject fit if too far away from last fit but recover after several frames
        """ TODO
        if (self.fits_rejected_in_a_row_count < 10) and \
